#!/usr/bin/env python3
"""Build ron-burgunding-cma from one locked toast panel and one rigid arm/glass rig."""
...
SLUG = "ron-burgunding-cma"
RAW_SHA256 = "3036d68b63f0b936467ce514f031a2fe1f92c25afd3229b2b252b7b395688f0f"
SELECTED_PANEL = 3
CELL = 627
SHARED_SCALE = 0.78
...
ARM_GLASS_POLYGON_RAW = (
    (165, 108), (245, 108), (250, 190), (220, 215), (205, 270),
    (180, 310), (135, 410), (75, 415), (44, 382), (50, 325),
    (105, 240), (125, 150),
)
PIVOT_RAW = (207, 268)
JOINT_RADIUS_RAW = 27
STATIC_RECTS_RAW = ((240, 22, 355, 200), (250, 215, 390, 570))
...
def assert_locked_sequence(
    ...
    if len(set(hashes)) != 4:
        raise ValueError("the rigid glass phases are not four unique cells")
    ...
    if len(set(static_hashes)) != 1:
        raise ValueError("face/torso registration changed outside the glass rig")
    changed = np.zeros((CELL, CELL), dtype=bool)
    for array in arrays[1:]:
        changed |= np.any(array != arrays[0], axis=2)
    escaped = changed & ~(np.asarray(allowed) > 0)
    if escaped.any():
        raise ValueError(f"{int(escaped.sum())} changed pixels escaped the glass rig")
    margins: list[int] = []
    for frame in frames:
        bbox = frame.getchannel("A").getbbox()
        if bbox is None:
            raise ValueError("empty glass phase")
    ...
    return {
        ...
        "selected_panel_role": "only immutable face, torso, suit, arm, hand and tumbler source",
