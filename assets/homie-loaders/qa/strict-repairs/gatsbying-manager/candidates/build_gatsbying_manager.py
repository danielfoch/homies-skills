#!/usr/bin/env python3
"""Build gatsbying-manager from one locked toast panel and one rigid arm/glass rig."""
...
SLUG = "gatsbying-manager"
RAW_SHA256 = "df1d0cc14aa84b2fae2e9946ecbdd7b750e3751fed24da445540a87fbb879ea9"
SELECTED_PANEL = 3
CELL = 627
SHARED_SCALE = 0.78
...
ARM_GLASS_POLYGON_RAW = (
    (155, 108), (240, 108), (243, 165), (216, 187), (210, 240),
    (181, 286), (156, 397), (104, 412), (67, 371), (78, 328),
    (118, 255), (135, 195), (140, 145),
)
PIVOT_RAW = (207, 258)
JOINT_RADIUS_RAW = 26
STATIC_RECTS_RAW = ((240, 25, 330, 195), (250, 215, 380, 590))
...
def build_frames(plate: Image.Image) -> tuple[list[Image.Image], Image.Image]:
    shape = polygon_mask(
        plate.size, tuple(mapped_point(point) for point in ARM_GLASS_POLYGON_RAW)
    )
...
def assert_locked_sequence(
    ...
    if len(set(hashes)) != 4:
        raise ValueError("the rigid toast phases are not four unique cells")
    ...
    if len(set(static_hashes)) != 1:
        raise ValueError("face/torso registration changed outside the toast rig")
    changed = np.zeros((CELL, CELL), dtype=bool)
    for array in arrays[1:]:
        changed |= np.any(array != arrays[0], axis=2)
    escaped = changed & ~(np.asarray(allowed) > 0)
    if escaped.any():
        raise ValueError(f"{int(escaped.sum())} changed pixels escaped the toast rig")
    margins: list[int] = []
    for frame in frames:
        bbox = frame.getchannel("A").getbbox()
        if bbox is None:
            raise ValueError("empty toast phase")
    ...
    return {
        ...
        "selected_panel_role": "only immutable face, torso, tuxedo, arm, hand and glass source",
